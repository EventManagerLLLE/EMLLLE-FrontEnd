...
const EventList: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center min-h-screen bg-gray-100">
            ...
            {events.map((event) => (
                <div key={event.id} className="bg-white shadow-md rounded-lg p-8 mb-6 max-w-lg w-full">
                    <h2 className="text-3xl font-bold mb-4 text-center">{event.title}</h2>
                    <p><strong>Datum:</strong> {event.date}</p>
                    <p><strong>Publikt:</strong> {event.isPublic ? "Ja" : "Nej"}</p>
                        ...
                        {event.participants.length > 0 ? (
                            event.participants.map((participant) => (
                                <div key={participant.id} className="border p-4 mb-2 rounded">
                                    <p><strong>Namn:</strong> {participant.firstName} {participant.lastName}</p>
                                </div>
                            ))
                        ) : (
